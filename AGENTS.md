...
guidelines:
  ...
  - title: "Imports in Cursor (no Pylance)"
    description: >
      ...
      - Use the workspace snippets: type the prefix (e.g. acpschema, acphelpers, glidertypes, gliderconfig) and accept the suggestion to insert the import line, then change the symbol name.
      - Or ask Cursor: select the undefined symbol, then Cmd+K and request "Add the missing import for <symbol>".
      - Or copy the import from an existing file in the repo (e.g. acp.schema, acp.helpers, glider.core.*).
