...
from glider.cli.textual_ui.widgets.tools import ToolCallMessage, ToolResultMessage
from glider.cli.update_notifier import UpdateCache
from glider.core.config import GliderConfig
from glider.core.types import FunctionCall, LLMMessage, Role, ToolCall
...
async def test_ui_displays_messages_when_resuming_session(
    ...
    agent_loop = build_test_agent_loop(config=glider_config)
    ...
    app = build_test_glider_app(agent_loop=agent_loop)
...
async def test_ui_rebuilds_history_when_whats_new_is_shown(
    ...
    config = build_test_glider_config(enable_update_checks=True)
