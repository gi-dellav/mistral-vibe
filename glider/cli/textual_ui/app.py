...
from glider import __version__ as CORE_VERSION
from glider.cli.clipboard import copy_selection_to_clipboard
from glider.cli.commands import CommandRegistry
from glider.cli.narrator_manager import (
    NarratorManager,
    NarratorManagerPort,
    NarratorState,
)
from glider.cli.plan_offer.adapters.http_whoami_gateway import HttpWhoAmIGateway
from glider.cli.plan_offer.decide_plan_offer import (
    PlanInfo,
    decide_plan_offer,
    plan_offer_cta,
    plan_title,
...
from glider.cli.plan_offer.ports.whoami_gateway import WhoAmIGateway, WhoAmIPlanType
from glider.cli.terminal_setup import setup_terminal
...
from glider.cli.textual_ui.widgets.chat_input.text_area import ChatTextArea
from glider.cli.textual_ui.widgets.compact import CompactMessage
from glider.cli.textual_ui.widgets.config_app import ConfigApp
from glider.cli.textual_ui.widgets.context_progress import ContextProgress, TokenState
from glider.cli.textual_ui.widgets.discover_mcp_app import DiscoverMCPApp
from glider.cli.textual_ui.widgets.discover_skills_app import DiscoverSkillsApp
from glider.cli.textual_ui.widgets.feedback_bar import FeedbackBar
from glider.cli.textual_ui.widgets.load_more import HistoryLoadMoreRequested
from glider.cli.textual_ui.widgets.loading import LoadingWidget, paused_timer
from glider.cli.textual_ui.widgets.messages import (
    BashOutputMessage,
    ErrorMessage,
    InterruptMessage,
...
from glider.cli.textual_ui.widgets.path_display import PathDisplay
from glider.cli.textual_ui.widgets.proxy_setup_app import ProxySetupApp
from glider.cli.textual_ui.widgets.question_app import QuestionApp
from glider.cli.textual_ui.widgets.rewind_app import RewindApp
from glider.cli.textual_ui.widgets.session_picker import SessionPickerApp
from glider.cli.textual_ui.widgets.teleport_message import TeleportMessage
from glider.cli.textual_ui.widgets.tools import ToolResultMessage
from glider.cli.textual_ui.widgets.voice_app import VoiceApp
from glider.cli.textual_ui.windowing import (
...
from glider.cli.update_notifier import (
...
from glider.cli.update_notifier.update import do_update
from glider.cli.voice_manager import VoiceManager, VoiceManagerPort
from glider.cli.voice_manager.voice_manager_port import TranscribeState
from glider.core.agent_loop import AgentLoop, TeleportError
from glider.core.agents import AgentProfile
from glider.core.audio_player.audio_player import AudioPlayer
from glider.core.audio_recorder import AudioRecorder
from glider.core.autocompletion.path_prompt_adapter import render_path_prompt
from glider.core.config import GliderConfig
from glider.core.data_retention import DATA_RETENTION_MESSAGE
from glider.core.logger import logger
from glider.core.paths import HISTORY_FILE
from glider.core.rewind import RewindError
...
from glider.core.session.session_loader import SessionLoader
...
from glider.core.tools.permissions import RequiredPermission
from glider.core.transcribe import make_transcribe_client
...
class GliderApp(App):  # noqa: PLR0904
    ...
    async def _handle_voice_settings_closed(
        ...
        if "voice_mode_enabled" in changes:
            ...
            if current != desired:
                ...
                self.agent_loop.telemetry_client.send_telemetry_event(
                    "glider.voice_mode_toggled", {"enabled": desired}
                )
        ...
        if non_voice_changes:
            GliderConfig.save_updates(non_voice_changes)
            self.agent_loop.refresh_config()
    ...
    async def _teleport(self, prompt: str | None = None) -> None:
        ...
        try:
            gen = self.agent_loop.teleport_to_glider_nuage(prompt)
...
args = ["-y", "@upstash/context7-mcp"] """,
    ...
    async def _check_update(self) -> None:
        ...
        if self.config.enable_auto_update and await do_update():
            self.notify(
                f"{update_message_prefix}\nGlider was updated successfully. Please restart to use the new version.",
        ...
        message = f"{update_message_prefix}\nPlease update glider-code with your package manager"
    ...
    def action_suspend_with_message(self) -> None:
        ...
        with self.suspend():
            rprint(
                "Glider for Mistral has been suspended. Run [bold cyan]fg[/bold cyan] to bring Glider for Mistral back."
            )
...
def run_textual_ui(
    ...
    update_notifier = PyPIUpdateGateway(project_name="glider-code")
