...
from glider.cli.narrator_manager.narrator_manager_port import (
...
if TYPE_CHECKING:
    ...
    from glider.cli.turn_summary import TurnSummaryPort
    from glider.core.audio_player.audio_player_port import AudioPlayerPort
    from glider.core.config import GliderConfig
    from glider.core.tts.tts_client_port import TTSClientPort
    from glider.core.types import BaseEvent


class NarratorManager:
    def __init__(
        self, config_getter: Callable[[], GliderConfig], audio_player: AudioPlayerPort
    ) -> None:
    ...
    @staticmethod
    def _make_turn_summary(config: GliderConfig) -> NoopTurnSummary | TurnSummaryTracker:
    ...
    @staticmethod
    def _make_tts_client(config: GliderConfig) -> TTSClientPort | None:
        if not config.narrator_enabled:
            return None
        try:
