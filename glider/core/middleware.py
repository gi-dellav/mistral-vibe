...
class ContextWarningMiddleware:
    ...
    async def before_turn(self, context: ConversationContext) -> MiddlewareResult:
        ...
        if context.stats.context_tokens >= max_context * self.threshold_percent:
            ...
            warning_msg = f"<{GLIDER_WARNING_TAG}>You have used {percentage_used:.0f}% of your total context ({context.stats.context_tokens:,}/{max_context:,} tokens)</{GLIDER_WARNING_TAG}>"
...
def make_plan_agent_reminder(plan_file_path: str) -> str:
    return f"""<{GLIDER_WARNING_TAG}>Plan mode is active. You MUST NOT make any edits (except to the plan file below), run any non-readonly tools (including changing configs or making commits), or otherwise make any changes to the system. This supersedes any other instructions you have received.
...
4. When your plan is complete, call the exit_plan_mode tool to request user approval and switch to implementation mode</{GLIDER_WARNING_TAG}>"""


PLAN_AGENT_EXIT = f"""<{GLIDER_WARNING_TAG}>Plan mode has ended. If you have a plan ready, you can now start executing it. If not, you can now use editing tools and make changes to the system.</{GLIDER_WARNING_TAG}>"""

CHAT_AGENT_REMINDER = f"""<{GLIDER_WARNING_TAG}>Chat mode is active. The user wants to have a conversation -- ask questions, get explanations, or discuss code and architecture. You MUST NOT make any edits, run any non-readonly tools, or otherwise make any changes to the system. This supersedes any other instructions you have received. Instead, you should:
...
4. Focus on being informative and conversational -- your response IS the deliverable, not a precursor to action</{GLIDER_WARNING_TAG}>"""

CHAT_AGENT_EXIT = f"""<{GLIDER_WARNING_TAG}>Chat mode has ended. You can now use editing tools and make changes to the system.</{GLIDER_WARNING_TAG}>"""
