...
from glider.core.config.harness_files._paths import (
    GLOBAL_AGENTS_DIR,
    GLOBAL_PROMPTS_DIR,
    GLOBAL_SKILLS_DIR,
    GLOBAL_TOOLS_DIR,
)
from glider.core.paths import AGENTS_MD_FILENAME, GLIDER_HOME, walk_local_config_dirs_all
...
class HarnessFilesManager:
    ...
    def config_file(self) -> Path | None:
        ...
        if workdir is not None:
            candidate = workdir / ".glider" / "config.toml"
            if candidate.is_file():
                return candidate
        if "user" in self.sources:
            return GLIDER_HOME.path / "config.toml"
        return None
    ...
    def project_prompts_dirs(self) -> list[Path]:
        ...
        if workdir is None:
            return []
        candidate = workdir / ".glider" / "prompts"
    ...
    def load_user_doc(self) -> str:
        if "user" not in self.sources:
            return ""
        path = GLIDER_HOME.path / AGENTS_MD_FILENAME
        try:
