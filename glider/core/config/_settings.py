...
from glider.core.lsp.config import LSPConfig, get_default_lsp_config
from glider.core.paths import GLOBAL_ENV_FILE, SESSION_LOG_DIR
from glider.core.prompts import SystemPrompt
...
class GliderConfig(BaseSettings):
    ...
    nuage_task_queue: str | None = Field(default="shared-glider-nuage", exclude=True)
    nuage_api_key_env_var: str = Field(default="MISTRAL_API_KEY", exclude=True)
    nuage_project_name: str = Field(default="Glider", exclude=True)
    ...
    def _apply_global_auto_compact_threshold(self) -> GliderConfig:
        self.models = [
            model
    ...
    @model_validator(mode="after")
    def _check_compaction_model_provider(self) -> GliderConfig:
    ...
    @model_validator(mode="after")
    def _check_api_key(self) -> GliderConfig:
        try:
    ...
    @model_validator(mode="after")
    def _validate_model_uniqueness(self) -> GliderConfig:
    ...
    @model_validator(mode="after")
    def _validate_transcribe_model_uniqueness(self) -> GliderConfig:
        seen_aliases: set[str] = set()
        for model in self.transcribe_models:
            if model.alias in seen_aliases:
                raise ValueError(
    ...
    @model_validator(mode="after")
    def _validate_tts_model_uniqueness(self) -> GliderConfig:
        seen_aliases: set[str] = set()
        for model in self.tts_models:
            if model.alias in seen_aliases:
                raise ValueError(
    ...
    @model_validator(mode="after")
    def _check_system_prompt(self) -> GliderConfig:
