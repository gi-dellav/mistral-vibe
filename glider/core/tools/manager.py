...
from glider.core.lsp.manager import set_lsp_config_getter
from glider.core.logger import logger
from glider.core.paths import DEFAULT_TOOL_DIR, LSP_TOOL_DIR
from glider.core.tools.base import BaseTool, BaseToolConfig
from glider.core.tools.mcp import MCPRegistry
from glider.core.utils import name_matches
...
def _try_canonical_module_name(path: Path) -> str | None:
    """Extract canonical module name for glider package files.
    ...
    try:
        glider_idx = parts.index("glider")
    except ValueError:
        return None

    if glider_idx + 1 >= len(parts):
        return None

    module_parts = [p.removesuffix(".py") for p in parts[glider_idx:]]
    return ".".join(module_parts)


def _compute_module_name(path: Path) -> str:
    """Return canonical module name for glider files, hash-based synthetic name otherwise."""
    ...
    return f"glider_tools_discovered_{stem}_{path_hash}"
...
class ToolManager:
    ...
    def __init__(
        self,
        config_getter: Callable[[], GliderConfig],
    ...
    @property
    def _config(self) -> GliderConfig:
        return self._config_getter()

    def _compute_search_paths(self, config: GliderConfig) -> list[Path]:
