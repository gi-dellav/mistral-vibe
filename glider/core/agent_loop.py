...
from glider.cli.terminal_setup import detect_terminal
from glider.core.agents.manager import AgentManager
from glider.core.agents.models import AgentProfile, BuiltinAgentName
...
from glider.core.llm.exceptions import BackendError
from glider.core.llm.format import (
...
from glider.core.prompts import UtilityPrompt
from glider.core.rewind import RewindManager
from glider.core.session.session_logger import SessionLogger
from glider.core.session.session_migration import migrate_sessions_entrypoint
from glider.core.skills.manager import SkillManager
from glider.core.system_prompt import get_universal_system_prompt
from glider.core.telemetry.send import TelemetryClient
from glider.core.tools.base import (
    BaseTool,
    InvokeContext,
    ToolError,
    ToolPermission,
...
from glider.core.tools.mcp import MCPRegistry
from glider.core.tools.mcp_sampling import MCPSamplingHandler
from glider.core.tools.permissions import (
    ApprovedRule,
    PermissionContext,
    RequiredPermission,
)
from glider.core.tools.utils import wildcard_match
from glider.core.tracing import agent_span, set_tool_result, tool_span
from glider.core.trusted_folders import has_agents_md_file
...
try:
    from glider.core.teleport.teleport import TeleportService as _TeleportService
...
if TYPE_CHECKING:
    from glider.core.teleport.nuage import TeleportSession
    from glider.core.teleport.teleport import TeleportService
    from glider.core.teleport.types import TeleportPushResponseEvent, TeleportYieldEvent
...
class TeleportError(AgentLoopError):
    """Raised when teleport to Glider Nuage fails."""
...
class AgentLoop:
    ...
    def refresh_config(self) -> None:
        self._base_config = GliderConfig.load()
    ...
    def teleport_service(self) -> TeleportService:
        ...
        if self._teleport_service is None:
            ...
            self._teleport_service = _TeleportService(
                ...
                glider_config=self._base_config,
            )
        return self._teleport_service

    def teleport_to_glider_nuage(
        ...
        match result.action:
            case MiddlewareAction.STOP:
                yield AssistantEvent(
                    content=f"<{GLIDER_STOP_EVENT_TAG}>{result.reason}</{GLIDER_STOP_EVENT_TAG}>",
    ...
    async def reload_with_initial_messages(
        self,
        base_config: GliderConfig | None = None,
        max_turns: int | None = None,
        max_price: float | None = None,
