...
if TYPE_CHECKING:
    from glider.core.agent_loop import ToolDecision
...
class TelemetryClient:
    ...
    def send_tool_call_finished(
        ...
        *,
        ...
        self.send_telemetry_event("glider.tool_call_finished", payload)

    def send_user_copied_text(self, text: str) -> None:
        payload = {"text_length": len(text)}
        self.send_telemetry_event("glider.user_copied_text", payload)

    def send_user_cancelled_action(self, action: str) -> None:
        payload = {"action": action}
        self.send_telemetry_event("glider.user_cancelled_action", payload)

    def send_auto_compact_triggered(self) -> None:
        payload = {}
        self.send_telemetry_event("glider.auto_compact_triggered", payload)
    ...
    def send_slash_command_used(
        ...
        self.send_telemetry_event("glider.slash_command_used", payload)
    ...
    def send_new_session(
        ...
        self.send_telemetry_event("glider.new_session", payload)

    def send_onboarding_api_key_added(self) -> None:
        self.send_telemetry_event(
            "glider.onboarding_api_key_added", {"version": __version__}
        )

    def send_user_rating_feedback(self, rating: int, model: str) -> None:
        self.send_telemetry_event(
            "glider.user_rating_feedback",
