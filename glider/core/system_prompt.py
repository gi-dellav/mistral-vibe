...
from glider.core.utils import is_dangerous_directory, is_windows

if TYPE_CHECKING:
    from glider.core.agents import AgentManager
    from glider.core.config import ProjectContextConfig, GliderConfig
...
def _add_commit_signature() -> str:
    return (
        ...
        "It will always be suffixed with a line telling it was generated by Glider for Mistral with the appropriate co-authoring information.\n"
        "The format you will always uses is the following heredoc.\n\n"
        "```bash\n"
        "git commit -m <Commit message here>\n\n"
        "Generated by Glider for Mistral.\n"
        "Co-Authored-By: Glider for Mistral <glider@mistral.ai>\n"
        "```"
    )
...
def get_universal_system_prompt(
    ...
    if config.include_project_context:
        ...
        if user_doc.strip():
            doc_sections.append(
                f"## User instructions\n\nContents of {GLIDER_HOME.path}/AGENTS.md (user-level instructions):\n\n{user_doc.strip()}"
            )
        if project_docs:
