"""Structured ACP error classes for the Glider agent.
...
and ACP error handling (https://agentclientprotocol.com/protocol/overview#error-handling):
  ...
  -31xxx            Application errors (Glider-specific, outside reserved range)
"""
...
from glider.core.config import MissingAPIKeyError
from glider.core.types import RateLimitError as CoreRateLimitError
...
# Glider application codes (outside JSON-RPC reserved range)
RATE_LIMITED = -31001
CONFIGURATION_ERROR = -31002
CONVERSATION_LIMIT = -31003


class GliderRequestError(RequestError):
    code: int
...
class UnauthenticatedError(GliderRequestError):
...
class NotImplementedMethodError(GliderRequestError):
...
class InvalidRequestError(GliderRequestError):
    code = INVALID_PARAMS
...
class SessionNotFoundError(GliderRequestError):
    code = INVALID_PARAMS
...
class SessionLoadError(GliderRequestError):
    code = INVALID_PARAMS
...
class RateLimitError(GliderRequestError):
    code = RATE_LIMITED
...
class ConfigurationError(GliderRequestError):
...
class ConversationLimitError(GliderRequestError):
...
class InternalError(GliderRequestError):
    code = INTERNAL_ERROR
