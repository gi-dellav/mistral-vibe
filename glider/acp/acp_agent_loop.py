...
from glider import GLIDER_ROOT, __version__
from glider.acp.acp_logger import acp_message_observer
from glider.acp.exceptions import (
    ConfigurationError,
...
from glider.acp.tools.base import BaseAcpTool
from glider.acp.tools.session_update import (
...
from glider.core.agents.models import CHAT as CHAT_AGENT, BuiltinAgentName
...
class GliderAcpAgentLoop(AcpAgent):
    client: Client
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        #  - dev mode: `uv run glider-acp`, ran from the project root
        #  - uv tool install: `glider-acp`, similar to dev mode, but uv takes care of path resolution
        #  - bundled binary: `./glider-acp` from binary location
        ...
        # So sys.executable is already /path/to/binary/glider-acp.
        ...
        auth_methods: list[EnvVarAuthMethod | TerminalAuthMethod | AuthMethodAgent] = (
            [
                TerminalAuthMethod(
                    ...
                    description="Register your API Key inside Glider for Mistral",
                    field_meta={
                        "terminal-auth": {
                            "command": command,
                            "args": args,
                            "label": "Glider for Mistral Setup",
                        }
                    },
                )
            ]
        ...
        response = InitializeResponse(
            ...
            agent_info=Implementation(
                name="@mistralai/glider-code",
                title="Glider for Mistral",
                version=__version__,
            ),
    ...
    def _load_config(self) -> GliderConfig:
        try:
            config = GliderConfig.load(disabled_tools=["ask_user_question"])
    ...
    def _get_acp_tool_overrides(self) -> list[Path]:
        ...
        return [
            GLIDER_ROOT / "acp" / "tools" / "builtins" / f"{override}.py"
        ...
        try:
            if not args:
                ...
                if value is not None:
                    ...
                    message = f"Set `{key}={value}` in ~/.glider/.env\n\nPlease start a new chat for changes to take effect."
                else:
                    unset_proxy_var(key)
                    message = f"Removed `{key}` from ~/.glider/.env\n\nPlease start a new chat for changes to take effect."
    ...
    async def _handle_leanstall_command(
        ...
        if "lean" in current:
            ...
            GliderConfig.save_updates({"installed_agents": [*current, "lean"]})
            new_config = GliderConfig.load(
    ...
    async def _apply_model_change(self, session: AcpSessionLoop, model_id: str) -> bool:
        ...
        GliderConfig.save_updates({"active_model": model_id})
