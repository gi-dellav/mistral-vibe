...
from glider.setup.onboarding.base import OnboardingScreen

PROVIDER_HELP = {
    "mistral": ("https://console.mistral.ai/codestral/cli", "Mistral AI Studio")
}
CONFIG_DOCS_URL = (
    "https://github.com/mistralai/glider-code?tab=readme-ov-file#configuration"
)
...
class ApiKeyScreen(OnboardingScreen):
    ...
    def __init__(self) -> None:
        super().__init__()
        config = GliderConfig.model_construct()
    ...
    def _compose_config_docs(self) -> ComposeResult:
        yield Static("[dim]Learn more about Glider configuration:[/]")
        yield Horizontal(
            NoMarkupStatic("→ ", classes="link-chevron"),
    ...
    def _save_and_finish(self, api_key: str) -> None:
        ...
        if self.provider.backend == Backend.MISTRAL:
            try:
                telemetry = TelemetryClient(config_getter=GliderConfig)
