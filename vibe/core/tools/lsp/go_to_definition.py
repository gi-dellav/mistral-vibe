...
class GoToDefinitionArgs(BaseModel):
    ...
    context_lines: int = Field(default=3, description="Context lines to include around definition")
...
class GoToDefinition(BaseLSPTool[GoToDefinitionArgs, GoToDefinitionResult, LSPToolConfig, LSPToolState]):
    ...
    async def run(
        ...
        try:
            ...
            params = self._create_position_params(args.file_path, args.line, args.character)
            
            # Send request to LSP server
            response = await server_process.request("textDocument/definition", params)
            
            if not response:
                raise ToolError("No definition found for the symbol at the specified location.")
    ...
    def _extract_symbol_name(self, context: str) -> str:
        """Extract symbol name from context."""
