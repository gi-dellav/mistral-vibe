...
class LSPServerProcess:
    ...
    def __init__(
        self, config: LSPServerConfig, workspace_root: Path, request_timeout: int = 30
    ):
        self.config = config
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            lsp_binary = self.config.command[0]
            if not shutil.which(lsp_binary):
                raise FileNotFoundError(
                    f"LSP server binary not found: {lsp_binary}. "
                    f"Please install the LSP server (e.g., pip install pyright, npm install -g typescript-language-server)."
                )
            ...
            # Create stream readers/writers using asyncio subprocess APIs
            self.writer = asyncio.StreamWriter(
                self.process.stdin, protocol=None, loop=asyncio.get_event_loop()
            )

            # Start reading messages using process stdout directly
...
class LSPManager:
    ...
    async def ensure_server_for_file(self, file_path: str) -> LSPServerProcess:
        ...
        if server_key not in self.servers:
            ...
            server_process = LSPServerProcess(
                server_config, workspace_root, self.config.request_timeout
            )
    ...
    def _parse_lsp_diagnostic(
        ...
        try:
            ...
            return Diagnostic(
                ...
                + 1,  # LSP uses 0-based, we use 1-based
                character=int(
